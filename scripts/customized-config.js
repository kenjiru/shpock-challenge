...
switch (process.argv[2]) {
    ...
    case 'start':
        rewireModule('react-scripts-ts/scripts/start.js', loadCustomizer('../config/config-overrides.dev'));
        break;

    // The "build" script is run to produce a production bundle
    case 'build':
        rewireModule('react-scripts-ts/scripts/build.js', loadCustomizer('../config/config-overrides.prod'));
        break;
